...
from homeassistant.components.homeassistant.scene import EVENT_SCENE_UPDATED
...
async def test_reload_config_service(hass):
    ...
    hass.bus.async_listen(
        EVENT_SCENE_UPDATED, lambda event: test_updated_event.append(event)
    )
    ...
    assert len(test_updated_event) == 2
